...
basic_query:
	SELECT selparams {q.selattrs = $selparams.attrs;} FROM frparams {q.from = $frparams.tables;};

selparams
	returns[ std::vector <std::string> attrs ]:
	ATTRNAME {$attrs.push_back($ATTRNAME.text);} ( ',' ATTRNAME {$attrs.push_back($ATTRNAME.text);})*
	| ALL {$attrs.push_back("ALL");};

frparams
	returns[ std::vector <std::string> tables ]:
	TABLENAME {$tables.push_back($TABLENAME.text);} (
		',' TABLENAME {$tables.push_back($TABLENAME.text);}
	)*;

whparams
	returns[ std::vector <Predicate> preds ]:
	whparam {$preds.push_back($whparam.pr);} ( (AND | OR) rp = whparams 
        {
